...
checkSurfaces(new URL('../../',import.meta.url),inventory);
...
const files = ['extension/ui/primitives.js', ...inventory.surfaces.map(item=>item.controller), ...readdirSync(new URL('../../ui/',import.meta.url),{recursive:true}).filter(file => /\.tsx?$/.test(file)).map(file => 'ui/' + file)];
