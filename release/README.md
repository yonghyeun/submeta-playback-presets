...
현재 재생 위치를 기준으로 하는 GIF 기본 구간과 공통 UI를 포함합니다. [배포 기록](RELEASE_0.4.2.md)을 참고하세요.

# 0.4.1 패키지 업데이트

공통 재생 UI를 포함한 최신 패키지와 검증 범위는 [0.4.1 배포 기록](RELEASE_0.4.1.md)을 참고하세요. 스토어 제출·공개 상태는 별도로 기록합니다.
...
- 라이선스: [MIT](../LICENSE)
- 코드 기여: [PR 절차](../CONTRIBUTING.md). [공개 원격 저장소](https://github.com/yonghyeun/submeta-playback-presets) 연결 완료. PR 및 `Extension validation` 통과를 요구하는 main 보호 규칙 적용.

## 제출 전 남은 결정

1. 심사용 Submeta 계정·강의 접근 제공 방법. 기존 개인 계정 정보는 자료에 넣지 않았다.
2. 서명만 받은 뒤 소수 배포할지, experimental 공개로 제출할지.
3. 외부 기여 PR 검토 및 필수 CI 검사 운영.
