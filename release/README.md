...
현재 Codex 환경의 자동 테스트는 `./scripts/test-local.sh`로 실행한다. 패키지는 Python 표준 라이브러리만 사용하며 허용 목록의 10개 파일만 포함한다. 같은 소스에서 두 번 생성해 동일한 ZIP 해시를 확인했다. .env, 계정, 브라우저 프로필, 테스트 호환 코드, 실험용 코드와 스토어 초안은 설치 패키지에 포함하지 않는다.

## 제출 전 남은 결정

1. 게시자 표시 이름과 공개 지원 연락처.
2. 배포 라이선스: 예를 들어 MIT로 소스 재사용을 허용할지, 별도 조건을 사용할지 사용자 선택이 필요하다. 현재 임의로 라이선스를 부여하지 않았다.
3. 심사용 Submeta 계정·강의 접근 제공 방법. 기존 개인 계정 정보는 자료에 넣지 않았다.
4. 서명만 받은 뒤 소수 배포할지, experimental 공개로 제출할지.
5. 실제로 선택 언어가 없는 강의 표본 검증 여부. 해당 동작은 자동 테스트만 통과했고 실사이트 표본은 미확인이다.
