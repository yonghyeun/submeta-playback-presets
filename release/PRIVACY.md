...
현재 영상만 해제 상태는 해당 탭의 메모리에 유지합니다. 브라우저의 확장 관리 기능에서 확장을 제거하면 브라우저가 관리하는 확장 저장 데이터도 제거됩니다. 설치 방식에 따른 Firefox의 데이터 처리 동작은 브라우저 정책을 따릅니다.
...
Current-video suspension is kept in the current tab's memory. Extension data removal is managed by the browser when uninstalling the extension, subject to Firefox's behavior for the installation method used.
