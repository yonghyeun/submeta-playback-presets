(() => {
  ...
  let generation=0,running=false,queued=false,timer,deadline=0,labels=[],lastReport='';
  let speedBudget=P.budget(),captionBudget=P.budget(),speedBlocked=false,captionBlocked=false;
  ...
  function reset() {generation++;deadline=Date.now()+15000;speedBudget=P.budget();captionBudget=P.budget();speedBlocked=false;captionBlocked=false;lastReport='';}
  function report() {
    if (!token) return;
  ...
  function mediaChanged() {suspended=false;labels=[];reset();schedule();}
  ...
  async function apply() {
    ...
    try {
      ...
      if(!active){speedState=`${v.playbackRate}배 · ${suspended?'현재 영상 해제':'자동 적용 꺼짐'}`;captionState=labelNow() || '확인 중';}
      else {
        if(!prefs.manageSpeed)speedState=`${v.playbackRate}배 · 변경하지 않음`;
        else if(speedBlocked)speedState='반복 충돌 · 다시 적용 필요';
        else {
          if(v.playbackRate!==prefs.rate){
            if(!speedBudget.take(Date.now())){speedBlocked=true;speedState='반복 충돌 · 다시 적용 필요';}
            else {try{v.playbackRate=prefs.rate;}catch{speedState='배속 변경 미지원';}}
          }
      ...
      report();
      if(!valid())return;
      if(!labels.length && captionButton())await withOptions(()=>{},valid);
      if(!valid())return;
      if(!active || prefs.captions==='leave') {captionState=`${labelNow() || '확인 중'} · 변경하지 않음`;return;}
      if(captionBlocked){captionState='반복 충돌 · 다시 적용 필요';return;}
      ...
      if(!captionVerified(target)) {
        if(!captionBudget.take(Date.now())){captionBlocked=true;captionState='반복 충돌 · 다시 적용 필요';return;}
        captionState='적용 확인 중';report();
        await selectCaption(desired,valid);
      }
      if(!valid())return;
      if(captionVerified(target))captionState=matched ? (target==='Off'?'꺼짐 확인됨':`${target} 활성화 확인됨`) : '선택 언어 미제공 · 자막 꺼짐';
  ...
  window.addEventListener('message',event=>{
    ...
    if(token!==m.token){token=m.token;configured=false;suspended=false;labels=[];reset();}
  ...
  browser.runtime.onMessage.addListener(m=>{
    ...
    if(next.revision!==prefs.revision || next.enabled!==prefs.enabled || m.retry)reset();
    prefs=next;configured=true;suspended=m.suspended===true;detect();schedule(0);
