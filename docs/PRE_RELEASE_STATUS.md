...
**0.3.0 배포 후보·검증 결과·등록 초안을 준비했다. 외부 제출은 하지 않았다.** 게시자 정보와 라이선스, 심사용 접근 방식은 아직 확정되지 않았으므로 즉시 제출 가능한 상태로 간주하지 않는다.
...
| 패키지 검사 | 허용 파일 10개, ZIP CRC 검사 및 두 번 생성한 SHA-256 일치 |
...
- 게시자 표시 이름 및 공개 지원 연락처 미입력.
- 라이선스 미선택. 개발자가 임의로 오픈소스 권한을 부여하지 않음.
